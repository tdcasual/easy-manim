...
import type { VideoThreadIterationDetailResult, VideoThreadSurface } from "../../lib/videoThreadsApi";
...
export function SelectedVersionHero({
  ...
  const videoDownloadHref =
    ...
    (selectedTaskId ? `/api/tasks/${encodeURIComponent(selectedTaskId)}/artifacts/final_video.mp4` : null);
  const scriptDownloadHref =
    downloads?.script_download_url ||
    (selectedTaskId ? `/api/tasks/${encodeURIComponent(selectedTaskId)}/artifacts/current_script.py` : null);
  ...
  return (
    <section aria-label="Selected version">
      ...
      <p>{summary}</p>
      <p>Selected result: {selectedResult?.result_id || surface.current_focus.current_result_id || "none"}</p>
      <p>Selected iteration: {selectedIterationId || surface.current_focus.current_iteration_id || "none"}</p>
      ...
      <div>
        ...
        {validationReportDownloadHref ? (
          <a href={resolveApiUrl(validationReportDownloadHref) ?? undefined}>Download validation report</a>
        ) : null}
      </div>
