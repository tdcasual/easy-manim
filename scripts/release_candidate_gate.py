...
def evaluate_gate_result(payload: dict[str, Any], min_pass_rate: float) -> dict[str, Any]:
    reasons: list[str] = []
...
def run_release_candidate_gate(mode: str, data_dir: Path, min_pass_rate: float) -> dict[str, Any]:
    root = repo_root()
    if mode == "ci":
    ...
    with tempfile.TemporaryDirectory(prefix="easy-manim-rc-gate-", dir=scratch_parent) as tmp_dir:
        ...
        summary.update(evaluate_gate_result(summary, min_pass_rate=min_pass_rate))
        return summary
...
def main() -> None:
    ...
    summary = run_release_candidate_gate(args.mode, args.data_dir, args.min_pass_rate)
